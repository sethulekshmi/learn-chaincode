...
func (t *SimpleChaincode) open_trade(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	for i:=3; i < len(args); i++ {
		...
		trade_away.Value = args[i]
		trade_away.Number =  will_number
...
func (t *SimpleChaincode) perform_trade(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	for i := range trades.OpenTrades{
		...
		if trades.OpenTrades[i].Timestamp == timestamp{
			...
			if closersMoney.Value != trades.OpenTrades[i].Want.Value || closersMoney.Number != trades.OpenTrades[i].Want.Number {
				msg := "money in input does not meet trade requriements"
				fmt.Println(msg)
				return nil, errors.New(msg)
			}

			money, e := findMoney4Trade(stub, trades.OpenTrades[i].User, args[4], size)
			if(e == nil){
...
func findMoney4Trade(stub shim.ChaincodeStubInterface, user string, value string, number int )(m Money, err error){
	var fail Money;
...
func cleanTrades(stub shim.ChaincodeStubInterface)(err error){
	...
	for i:=0; i<len(trades.OpenTrades); {
		...
		for x:=0; x<len(trades.OpenTrades[i].Willing); {
			...
			_, e := findMoney4Trade(stub, trades.OpenTrades[i].User, trades.OpenTrades[i].Willing[x].Value, trades.OpenTrades[i].Willing[x].Number)
			if(e != nil){
